<?php
class vAgencia extends Eloquent {
    ...
    {        
        ...
        $sql = "select nombre, fecha, agencia, sum(venta) as venta, sum(premio) as premio, sum(venta)-sum(ifnull(premio,0)) as saldo 
               ...
               if ($agencia <> NULL)
                ...
                $sql = $sql." group by agencia order by fecha asc";
 ...
 public static function MakeResumenGrid($agencia, $desde, $hasta)
    {        
        ...
        $sql="SELECT agencia, nombre, fecha, vloteria, vparley, vpreimpreso, vcopia, ploteria, pparley, ppreimpreso, pcopia, 
                ifnull(vloteria,0)+ifnull(vparley,0)+ifnull(vpreimpreso,0)+ifnull(vcopia,0)-ifnull(ploteria,0)-ifnull(pparley,0)-ifnull(ppreimpreso,0)-ifnull(pcopia,0) as saldo 
                ...
                $sql = $sql." where agencia = '".$agencia."'";     
                 $sql = $sql." group by agencia ) as B ON A.id = B.dia order by fecha asc";
