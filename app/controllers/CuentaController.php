...
class CuentaController extends \BaseController {
	...
	public function index()
	{
		...
		$cuentas = DB::table('cuentas')->paginate(10, array('id', 'fecha', 'agencia', 'categoria', 'descripcion', 'debe', 'haber'));
	...
	public function createSingle()
	{
		...
		$max = DB::table('cuentas')->max('id');		
		$cuenta->id =$max+1;


		
		$fecha = Input::get('fecha_add');
		$fecha = date("Y-m-d", strtotime($fecha));

		$cuenta->fecha = $fecha;
		$cuenta->producto = Input::get('producto2');
		$cuenta->agencia = Input::get('agencia2');
		$cuenta->categoria = Input::get('categoria2');            
		$cuenta->descripcion = Input::get('descripcion2');

		if (   $cuenta->categoria == 'Venta' or $cuenta->categoria == 'Caja'  )
		$cuenta->debe = Input::get('monto2');
		else 
		$cuenta->haber = Input::get('monto2');

		if (   $cuenta->categoria == 'RRHH' )
		$cuenta->producto = 'RRHH';                
